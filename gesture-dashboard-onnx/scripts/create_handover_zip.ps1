...
$ArchivePath = Join-Path $ParentDirectory "gesture-dashboard-onnx-handover-$Timestamp.zip"
$TemporaryRoot = [System.IO.Path]::GetFullPath([System.IO.Path]::GetTempPath())
$StagingDirectory = [System.IO.Path]::GetFullPath(
    (Join-Path $TemporaryRoot "gesture-dashboard-onnx-handover-$Timestamp")
)
$PackageDirectory = Join-Path $StagingDirectory "gesture-dashboard-onnx"
...
Write-Host "Created ONNX handover package:" -ForegroundColor Green
