...
def artifact_status(models_directory: Path = MODELS_DIRECTORY) -> list[dict[str, Any]]:
    expected = [
        ...
        ("Joblib runtime configuration", "gesture_joblib_runtime_config.json", True),
        ("Primary Python classifier", "gesture_primary_model.joblib", False),
        ("SVM/MLP model bundle", "*models.joblib", False),
        ("Online Python checkpoint", "gesture_online_state.joblib", False),
        ("Online replay cache", "gesture_online_replay_cache.npz", False),
        ("Online validation cache", "gesture_online_validation_cache.npz", False),
        ("Untouched test cache", "gesture_online_untouched_test_cache.npz", False),
        ("Artifact integrity manifest", "gesture_artifact_manifest.json", True),
        ("Production qualification", "gesture_production_qualification.json", False),
    ]
    rows = []
