...
from .release_qualification import ReleaseQualifier
...
class RollbackPayload(BaseModel):
    backup_name: str = Field(min_length=1, max_length=180, pattern=r"^[A-Za-z0-9_.-]+$")
...
def _build_services() -> tuple[
    ...
    learning = OnlineLearningService(
        ...
        force_learning_enabled=production_settings.allow_force_learning,
        promote_updates_live=not production_settings.production_mode,
    )
    qualifier = ReleaseQualifier(config, production_settings)
    return config, inference_engine, store, learning, qualifier


runtime_config, engine, feedback_store, online_learning, release_qualifier = _build_services()
...
app = FastAPI(
    title="Gesture Control Lab API",
    version="2.0.0",
    description="Production-hardened local MediaPipe + SVM/MLP gesture runtime.",
...
def current_health() -> dict[str, Any]:
    ...
    release = release_qualifier.status(engine.model_manager.selected_model_name)
    return {
        ...
        "online_learning": online_learning.status(),
        "production": {
...
def production_qualification() -> dict[str, Any]:
    return release_qualifier.status(engine.model_manager.selected_model_name)
...
async def run_production_qualification(
    ...
    async with service_lock:
        return await asyncio.to_thread(release_qualifier.run, engine.model_manager)
...
def save_feedback(payload: FeedbackPayload) -> dict[str, Any]:
    try:
        ...
        update: dict[str, Any] = {
            "status": "audit_only",
            "message": "Feedback saved without changing the live model.",
        }
        if learning_mode == "force" and not production_settings.allow_force_learning:
            update = {
                "status": "disabled",
                "message": (
                    "Feedback was saved, but Force Learn is disabled in production. "
                    "Use validation-gated Safe Learn."
                ),
            }
        elif learning_mode in {"safe", "force"}:
            if not payload.feature_vector:
                raise ValueError("A live 76-D feature vector is required to learn.")
            try:
                update = online_learning.learn(
                    payload.feature_vector,
                    payload.actual_label,
                    force=learning_mode == "force",
                )
            except (ValueError, TypeError, OSError, RuntimeError, PermissionError) as error:
                update = {
                    "status": "error",
                    "message": "Feedback was saved, but the model update failed: " + str(error),
                }
...
async def reload_artifacts(
    ...
    global runtime_config, engine, feedback_store, online_learning, release_qualifier
    _admin_allowed(production_settings.allow_artifact_reload, x_admin_token)
    async with service_lock:
        old_engine = engine
        runtime_config, engine, feedback_store, online_learning, release_qualifier = _build_services()
...
def root() -> dict[str, str]:
    return {
        "name": "Gesture Control Lab API",
        "version": app.version,
        "dashboard": "Start the frontend and open http://127.0.0.1:3000",
