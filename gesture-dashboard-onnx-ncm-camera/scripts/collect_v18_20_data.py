...
Run from the project root: python -m scripts.collect_v18_20_data
"""
...
def collect_hagrid(destination: Path, train_limit=2000, eval_limit=400):
    from remotezip import RemoteZip
    destination.mkdir(parents=True, exist_ok=True)
    # Several unknown shapes, including genuine neutral hands, are retained.
    names = ['one', 'palm', 'like', 'ok', 'rock', 'fist', 'call', 'peace', 'no_gesture']
    with RemoteZip(HAGRID_URL, timeout=90) as archive:
        for split in ['train', 'val', 'test']:
            ...
            for name in names:
                ...
                if name in ['rock', 'fist', 'call', 'peace', 'no_gesture']:
                ...
                for key in keys:
                    ...
                    for hand, (label, landmarks) in enumerate(zip(record['labels'], record.get('hand_landmarks', []))):
                        ...
                        if name == 'one':
                            ...
                            rows.append(dict(base, points=points, label={'palm': 4, 'like': 5, 'ok': 7}.get(name, 8)))
                        count += 1
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=Path('data/v18_20'))
    parser.add_argument('--source', choices=['all', 'hagrid', 'dorsal'], default='all')
    args = parser.parse_args()
    args.output.mkdir(parents=True, exist_ok=True)
    if args.source in ['all', 'hagrid']:
        collect_hagrid(args.output)
